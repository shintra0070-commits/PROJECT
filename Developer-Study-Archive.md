...
지금까지 진행한 AI 프로젝트와 모델 개발 과정에서 해결했던 대표적인 문제들을 정리한 기록입니다.

## 1. 온프레미스 AX/DX 기반의 민원 업무 자동화 플랫폼 구축

### 프로젝트 개요

- Ollama 기반 LLM을 활용한 온프레미스 민원 자동 분류 시스템 개발
- 단일 라벨 및 다중 라벨 민원 자동 분류 지원

### 문제 해결

- **민원 분류 정확도 향상**
  - 문의형 민원과 복합 민원의 오분류 발생
  - Rule-based 후처리와 프롬프트 개선을 적용하여 분류 정확도 향상

- **다중 라벨 분류 적용**
  - 하나의 민원이 여러 업무에 해당하는 경우 단일 분류의 한계 발생
  - 다중 라벨 분류 규칙을 적용하여 실제 업무 프로세스에 맞도록 개선

- **LLM 성능 비교**
  - Gemma, Llama, Qwen 등 다양한 LLM을 비교
  - Accuracy, Weighted F1, 처리시간을 분석하여 최적 모델 선정

- **데이터 불균형 개선**
  - 클래스별 데이터 수 차이로 성능 편차 발생
  - 데이터 추가 구축 및 클래스 균형 조정을 통해 성능 개선
...
- **클래스 불균형 문제**
  - 심근경색 데이터가 적은 불균형 데이터를 SMOTE로 보정하여 모델 학습 성능을 개선하였다.

- **변수 선택 문제**
  - 카이제곱 검정과 Mann-Whitney U Test를 활용하여 통계적으로 유의한 변수만 선정하였다.

- **이상치 처리 문제**
  - 공복혈당 이상치가 발견되었지만 교차 검증을 통해 실제 초고위험군 데이터로 판단하여 제거하지 않고 학습에 활용하였다.

- **모델 최적화**
  - Random Forest, LightGBM, XGBoost를 비교하고 GridSearchCV를 통해 최적의 하이퍼파라미터를 탐색하여 최고 ROC-AUC 0.9801을 달성하였다.
...
- **예측 성능 저하 문제**
  - 외생 변수만으로는 예측 성능이 낮아 종속변수(BDI)의 과거값(Lag)을 추가하여 시계열 특성을 반영하였다.

- **학습 데이터 부족 문제**
  - 월별 데이터의 표본 수가 부족하여 일별 데이터로 재구성하고 데이터 수를 증가시켜 학습 안정성과 예측 정확도를 개선하였다.

- **다중공선성 문제**
  - VIF가 높은 변수가 존재했지만 시계열 특성을 고려하여 제거하지 않고 모델링을 수행하였다.

- **모델 최적화**
  - 여러 회귀 모델을 비교한 후 GradientBoosting을 최종 모델로 선정하고 GridSearchCV를 활용하여 하이퍼파라미터를 최적화하였다.
